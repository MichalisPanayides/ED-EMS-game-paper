...
for lambda_2 in lambda_2_space:
    ...
    all_sim_waiting_times.append([np.mean(w.waiting_times) for w in simulation_results])
...
for index, (ax, all_markov_times) in enumerate(
    ...
    ax.violinplot(
        all_sim_waiting_times, positions=lambda_2_space, showmeans=True, widths=0.3
    )
    ax.plot(
        lambda_2_space,
        all_markov_times,
